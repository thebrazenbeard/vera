...
`architecture/portfolio/VERA_PORTFOLIO_ABSORPTION_V1.json` is bound to the immutable 67-repository Project Runner cut observed on 2026-09-24. Public source names all 49 public members; the 18 private members are represented count-only. `architecture/portfolio/VERA_PORTFOLIO_CAPABILITY_ARCHITECTURE_V1.json` maps every named public member into one or more internal Vera capability planes. `architecture/portfolio/VERA_PORTFOLIO_HARVEST_V1.json` records the public implementation disposition, and `architecture/portfolio/VERA_PORTFOLIO_MIGRATION_BINDINGS_V1.json` preserves exact public donor provenance while reducing private donor provenance to aggregate counts. The cut is immutable evidence, not permanent world cardinality: later repository drift is recorded as freshness metadata and must be refreshed before currentness, scheduling, or effect claims.
