...
def _worker(args: argparse.Namespace) -> int:
    input_data = _load_object(
        args.input,
        expected_fields={"state", "state_attestations", "trusted_state_keys", "trusted_lifecycle_keys"},
    )
    ...
    checkpoint = write_checkpoint(
        ...
        trusted_state_keys=input_data["trusted_state_keys"],
    ...
    termination = write_termination_intent(
        ...
        trusted_lifecycle_keys=input_data["trusted_lifecycle_keys"],
        lifecycle_issuer=issuer,
        lifecycle_key_id=key_id,
        lifecycle_signer=signer,
...
def _recover(args: argparse.Namespace) -> int:
    required = {
        ...
        "expected_lifecycle_registry_head",
    }
    ...
    receipt = recover(
        ...
        trusted_temporal_keys=input_data["trusted_temporal_keys"],
        ...
        trusted_lifecycle_keys=input_data["trusted_lifecycle_keys"],
        trusted_supervisor_keys=input_data["trusted_supervisor_keys"],
        trusted_state_keys=input_data["trusted_state_keys"],
        ...
        lifecycle_registry_path=input_data["lifecycle_registry_path"],
        lifecycle_registry_key=bytes.fromhex(os.environ["VERA_R8A0_REGISTRY_KEY_HEX"]),
        expected_lifecycle_registry_head=input_data[
            "expected_lifecycle_registry_head"
        ],
    )
