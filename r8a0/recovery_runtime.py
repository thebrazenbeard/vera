...
def recover(
    ...
    *,
    ...
    trusted_temporal_keys: Mapping[str, Mapping[str, Any]],
    ...
    lifecycle_registry_key: bytes,
    expected_lifecycle_registry_head: str,
) -> dict[str, Any]:
    ...
    orientation = OrientationGate(trusted_source_keys=trusted_temporal_keys).evaluate(
    ...
    termination_intent = read_termination_intent(
        termination_intent_path, trusted_lifecycle_keys=trusted_lifecycle_keys
    )
    exit_attestation = read_exit_attestation(
        exit_attestation_path, trusted_supervisor_keys=trusted_supervisor_keys
    )
    if (
    ...
    claim = {
        "schema": "VERA_R8A0_RUNTIME_RESUMPTION_CLAIM_V1",
    ...
    registry = LifecycleRegistry(lifecycle_registry_path, lifecycle_registry_key)
    registry_head = registry.consume(
        ...
        expected_head=expected_lifecycle_registry_head,
    )
    body = {
        "schema": "VERA_R8A0_RUNTIME_RESUMPTION_RECEIPT_V5",
        ...
        "exit_attestation_signature": exit_attestation["signature"],
        "resumption_claim_digest": claim_digest,
        "lifecycle_registry_head": registry_head,
