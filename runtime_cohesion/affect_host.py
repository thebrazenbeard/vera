...
class VeraAffectiveRuntimeHost:
    ...
    def _receipt_with_runtime_implementation_cut(self, receipt: Mapping[str, Any]) -> dict[str, Any]:
        ...
        existing_cut = normalized.get("runtime_implementation_cut")
        if existing_cut is not None and existing_cut != self.runtime_implementation_cut:
            raise AffectiveBindingError("event receipt runtime implementation cut does not match active Vera source")
