...
def _validate_consumed_evidence_provenance(
    ...
    *,
    ...
    if not isinstance(provenance, Mapping):
        raise AffectiveReceiptSemanticError(f"{label} receipt requires structured verified provenance")
    ...
    for field in (
        ...
        if provenance.get(field) != subject.get(field):
            raise AffectiveReceiptSemanticError(f"{label} provenance {field} does not match consumed subject")
...
def validate_affective_event_receipt(
    ...
    *,
    ...
    Historical/nonqualifying receipts can be checked for exact internal
    consistency. V1 intentionally has no source-local production trust root.
    Consequently any attempt to use a post-event receipt as production/current
    evidence fails closed until independently rooted authority composition is
    integrated outside this caller-controlled Python process.
    """
    ...
    if require_engineered_claim:
        raise AffectiveReceiptSemanticError(
            "production/current post-event validation requires an independently rooted authority capability not implemented by this source package"
        )
