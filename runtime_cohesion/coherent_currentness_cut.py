...
Retry continuity is structurally bound through the supplied predecessor cut itself.
The source evaluator can verify lineage and prior disposition over that full cut, but
does not prove external execution, persistence, admission, or authoritative custody.
"""
...
class CoherentCurrentnessCut:
    ...
    retry_count: int
    predecessor_cut: "CoherentCurrentnessCut | None"
    ...
    def __post_init__(self) -> None:
        ...
        if self.retry_count == 0:
            if self.predecessor_cut is not None:
                raise ValueError(
                    "initial currentness cut cannot claim predecessor_cut"
                )
        else:
            if type(self.predecessor_cut) is not CoherentCurrentnessCut:
                raise ValueError(
                    "retry currentness cut requires exact predecessor CoherentCurrentnessCut"
                )
            predecessor = self.predecessor_cut
            if predecessor.retry_count != 0 or predecessor.predecessor_cut is not None:
                raise ValueError("predecessor cut must be an initial cut")
            if predecessor.cut_family_id != self.cut_family_id:
                raise ValueError("predecessor cut_family_id must match retry cut family")
            if predecessor.requirement_profile.digest != self.requirement_profile.digest:
                raise ValueError(
                    "predecessor requirement_profile digest must match retry profile"
                )
            if (
                predecessor.requirement_profile.scope_digest
                != self.requirement_profile.scope_digest
            ):
                raise ValueError("predecessor scope_digest must match retry scope")
            predecessor_decision = evaluate_currentness_cut(predecessor)
            if (
                predecessor_decision.disposition
                is not CutDisposition.RETRY_AFFECTED_SURFACES
            ):
                raise ValueError(
                    "predecessor cut must evaluate to RETRY_AFFECTED_SURFACES"
                )
    ...
    def payload(self) -> dict[str, Any]:
        return {
            ...
            "predecessor_cut_digest": (
                None if self.predecessor_cut is None else self.predecessor_cut.digest
            ),
            "predecessor_cut": (
                None if self.predecessor_cut is None else self.predecessor_cut.payload()
            ),
