...
comment on column public.vera_affective_runtime_state_v1.checkpoint_sha256 is
  'Canonical SHA-256 of the exported Vera affective checkpoint. Restore additionally requires a separately pinned expected digest; this column alone is not the trust boundary.';

create unique index if not exists vera_affective_runtime_events_v1_event_digest_uq
  on public.vera_affective_runtime_events_v1(event_digest);
...
comment on function public.vera_affective_runtime_commit_v1(bigint, jsonb, jsonb) is
  'Atomic compare-and-swap commit for Vera affective runtime state plus zero or more append-only event rows. A stale expected_prior_version aborts the entire transaction.';
...
-- Force provider writes through the CAS function. service_role retains readback.
revoke insert, update on table public.vera_affective_runtime_state_v1 from service_role;
revoke insert on table public.vera_affective_runtime_events_v1 from service_role;
grant select on table public.vera_affective_runtime_state_v1 to service_role;
grant select on table public.vera_affective_runtime_events_v1 to service_role;
