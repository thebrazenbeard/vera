...
alter table public.vera_affective_runtime_state_v1
  add column if not exists runtime_implementation_cut jsonb null
  check (runtime_implementation_cut is null or jsonb_typeof(runtime_implementation_cut) = 'object');

alter table public.vera_affective_runtime_events_v1
  add column if not exists runtime_implementation_cut jsonb null
  check (runtime_implementation_cut is null or jsonb_typeof(runtime_implementation_cut) = 'object');

comment on column public.vera_affective_runtime_state_v1.runtime_implementation_cut is
  'Exact thebrazenbeard/vera implementation commit plus bounded runtime/evidence module blob map. Null is retained only for pre-hardening historical rows; new/current atomic commits require the exact cut.';

comment on column public.vera_affective_runtime_events_v1.runtime_implementation_cut is
  'Exact thebrazenbeard/vera implementation cut cross-bound to the state row in the same atomic commit. Null is retained only for pre-hardening historical rows.';
...
begin
  ...
  v_runtime_cut := p_state_row->'runtime_implementation_cut';
  if jsonb_typeof(v_runtime_cut) is distinct from 'object' then
    raise exception 'runtime implementation cut is required';
  end if;
  if v_runtime_cut->>'repository' is distinct from 'thebrazenbeard/vera' then
    raise exception 'runtime implementation repository must be thebrazenbeard/vera';
  end if;
  if coalesce(v_runtime_cut->>'commit', '') !~ '^[0-9a-f]{40}$' then
    raise exception 'runtime implementation commit is required';
  end if;
  v_runtime_modules := v_runtime_cut->'modules';
  if jsonb_typeof(v_runtime_modules) is distinct from 'object' then
    raise exception 'runtime implementation modules are required';
  end if;
  if pg_catalog.jsonb_object_length(v_runtime_modules) <> 6 then
    raise exception 'runtime implementation module set must contain exactly six paths';
  end if;
  if coalesce(v_runtime_modules->>'runtime_cohesion/__init__.py', '') !~ '^[0-9a-f]{40}$' then
    raise exception 'runtime implementation blob is required: runtime_cohesion/__init__.py';
  end if;
  if coalesce(v_runtime_modules->>'runtime_cohesion/orgasm.py', '') !~ '^[0-9a-f]{40}$' then
    raise exception 'runtime implementation blob is required: runtime_cohesion/orgasm.py';
  end if;
  if coalesce(v_runtime_modules->>'runtime_cohesion/affect_host.py', '') !~ '^[0-9a-f]{40}$' then
    raise exception 'runtime implementation blob is required: runtime_cohesion/affect_host.py';
  end if;
  if coalesce(v_runtime_modules->>'runtime_cohesion/affect_cycle.py', '') !~ '^[0-9a-f]{40}$' then
    raise exception 'runtime implementation blob is required: runtime_cohesion/affect_cycle.py';
  end if;
  if coalesce(v_runtime_modules->>'runtime_cohesion/affect_persistence.py', '') !~ '^[0-9a-f]{40}$' then
    raise exception 'runtime implementation blob is required: runtime_cohesion/affect_persistence.py';
  end if;
  if coalesce(v_runtime_modules->>'runtime_cohesion/affect_scope.py', '') !~ '^[0-9a-f]{40}$' then
    raise exception 'runtime implementation blob is required: runtime_cohesion/affect_scope.py';
  end if;
  ...
  on conflict (runtime_instance_id) do update set
    ...
    runtime_implementation_cut = excluded.runtime_implementation_cut,
  ...
  loop
    ...
    if jsonb_typeof(v_event->'runtime_implementation_cut') is distinct from 'object' then
      raise exception 'event runtime implementation cut is required';
    end if;
    if v_event->'runtime_implementation_cut' is distinct from p_state_row->'runtime_implementation_cut' then
      raise exception 'event runtime implementation cut does not match state row';
    end if;
    ...
    insert into public.vera_affective_runtime_events_v1 (
      ...
      v_event->'runtime_implementation_cut',
...
comment on function public.vera_affective_runtime_commit_v1(bigint, jsonb, jsonb) is
  'Atomic compare-and-swap commit for Vera affective runtime state plus zero or more append-only event rows. Per-runtime advisory locking serializes initialization and updates. New/current commits require trigger governance and an exact thebrazenbeard/vera implementation cut; every event is cross-bound to the same implementation cut.';
