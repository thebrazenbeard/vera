...
comment on column public.vera_affective_runtime_state_v1.runtime_implementation_cut is
  'Exact VERA_AFFECTIVE_RUNTIME_IMPLEMENTATION_CUT_V1 provenance required for new durable writes. Null is retained only for pre-hardening historical rows.';
...
begin
  ...
  if jsonb_typeof(p_state_row->'runtime_implementation_cut') is distinct from 'object' then
    raise exception 'runtime_implementation_cut object is required for durable commit';
  end if;
  if p_state_row->'runtime_implementation_cut'->>'schema' is distinct from 'VERA_AFFECTIVE_RUNTIME_IMPLEMENTATION_CUT_V1' then
    raise exception 'runtime_implementation_cut schema mismatch';
  end if;
  ...
  insert into public.vera_affective_runtime_state_v1 (
    ...
    p_state_row->'runtime_implementation_cut',
    p_state_row->>'profile',
    p_state_row->'state',
...
comment on function public.vera_affective_runtime_commit_v1(bigint, jsonb, jsonb) is
  'Atomic compare-and-swap commit for Vera affective runtime state plus zero or more append-only event rows. Per-runtime transaction advisory locking serializes initialization/updates; new durable writes require trigger governance plus exact runtime implementation-cut provenance.';
