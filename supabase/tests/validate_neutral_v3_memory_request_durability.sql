...
-- before this file runs. Additional hostile writes roll back.
...
begin
  ...
  recovered_receipt := public.append_vera_context_v3(
    'MREQ-durable-concurrent-save', concurrent_record
  );
  ...
  where request_id = 'MREQ-durable-concurrent-save';
  if record_count <> 1 or journal_count <> 1 then
    raise exception 'reuse conflict mutated durable state';
  end if;
end;
$$;
...
begin
  if not has_function_privilege(
       'service_role', 'public.append_vera_context_v3(text,jsonb)', 'EXECUTE'
     ) then
    raise exception 'service_role lacks governed durable append execute';
  end if;
  if has_function_privilege(
       'service_role', 'public.append_vera_context_v3_once(text,jsonb)', 'EXECUTE'
     ) then
    raise exception 'service_role can bypass durable request journaling';
  end if;
  if has_table_privilege(
       'service_role', 'public.vera_memory_request_receipts_v3', 'SELECT'
     ) or has_table_privilege(
       'service_role', 'public.vera_memory_request_receipts_v3', 'INSERT'
     ) or has_table_privilege(
       'service_role', 'public.vera_memory_request_receipts_v3', 'UPDATE'
     ) or has_table_privilege(
       'service_role', 'public.vera_memory_request_receipts_v3', 'DELETE'
     ) then
  ...
  exception when others then
    if position('canonical identifier' in sqlerrm) > 0 then
      blocked := true;
    else
      raise;
    end if;
  ...
  if not blocked then
    raise exception 'noncanonical request ID was accepted';
  end if;
end;
$$;
...
begin
  ...
  if not update_blocked or not delete_blocked then
    raise exception 'request journal is not append-only';
  end if;
end;
$$;
...
declare
  interrupted_record jsonb := $record$
  {
    ...
    "limitations": ["Synthetic CI record in a disposable database."]
  }
  $record$::jsonb;
...
begin
  begin
    perform public.append_vera_context_v3(
      'MREQ-durable-interrupted-save', interrupted_record
    );
  exception when others then
  ...
  if record_count <> 0 or journal_count <> 0 then
    raise exception 'interrupted request left partial state';
  end if;
end;
$$;
...
begin
  ...
  if first_receipt is distinct from retry_receipt then
    raise exception 'post-interruption retry did not recover stored receipt';
  end if;
  ...
  where request_id = 'MREQ-durable-interrupted-save';
  if record_count <> 1 or journal_count <> 1 then
    raise exception 'post-interruption retry did not commit exactly once';
  end if;
end;
$$;
