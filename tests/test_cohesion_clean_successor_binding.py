...
class CohesionCleanSuccessorBindingTests(unittest.TestCase):
    ...
    def test_runtime_binding_module_deletion_fails_closed(self):
        record = self.record()
        binding = load_json_strict(BINDING_PATH)
        binding["runtime_implementation_cut"]["modules"].pop(
            "runtime_cohesion/affect_scope.py"
        )
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "binding.json"
            path.write_text(json.dumps(binding), encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "runtime binding blob mismatch"):
                validate_clean_successor(
                    record,
                    ownership_path=OWNERSHIP_PATH,
                    binding_path=path,
    ...
    def test_source_registry_required_row_mutation_fails_closed(self):
        record = self.record()
        registry_path = (
            ROOT / "architecture/cohesion/VERA_COHESION_SOURCE_REGISTRY_V0_20260911.json"
        )
        registry = load_json_strict(registry_path)
        target = next(
            row
            for row in registry["sources"]
            if row["id"] == "vera-clean-successor-repair-generation"
        )
        target["observed_head"] = "0" * 40
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "registry.json"
            path.write_text(json.dumps(registry), encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "source registry blob mismatch"):
                validate_clean_successor(
                    record,
                    ownership_path=OWNERSHIP_PATH,
                    binding_path=BINDING_PATH,
    ...
    def test_repair_input_provenance_commits_are_normative(self):
        record = self.record()
        mutated = deepcopy(record)
        mutated["repair_input"]["registry_refresh_commit"] = "0" * 40
        with self.assertRaisesRegex(ValueError, "R2 repair-input provenance mismatch"):
            validate_clean_successor(
                mutated,
                ownership_path=OWNERSHIP_PATH,
                binding_path=BINDING_PATH,
