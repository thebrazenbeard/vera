...
class TrustedVerifier:
    ...
    def verify(self, subject, *, expected_referent, expected_effect_class):
        ...
        return {
            ...
            "evidence_id": "affective-cycle-evidence",
...
class VeraAffectiveCycleTests(unittest.TestCase):
    ...
    def subject(effect):
        return {
            ...
            "source": "trusted-affective-cycle-test",
            "observed_at": "2026-09-10T19:45:00+00:00",
            "currentness": "CURRENT",
            "expiry_or_supersession": None,
        }

    def test_process_turn_emits_signal_without_mutating_generic_planning_and_persists_checkpoint(self):
        cycle, state_rows, event_rows = self.make_cycle()
        planning = {"valuation": 0.2, "salience": 0.2, "attention": 0.2, "truth": 0.9}
        ...
        self.assertEqual(result.planning_context, planning)
    ...
    def test_admin_forced_cycle_is_causal_unrooted_and_emits_signal_without_planning_mutation(self):
        cycle, state_rows, event_rows = self.make_cycle()
        planning = {"valuation": 0.2, "salience": 0.2, "attention": 0.2, "truth": 0.9}
        ...
        provenance = result.event_receipt["trigger_provenance"]
        self.assertIsInstance(provenance, Mapping)
        self.assertEqual(provenance["authorization_subject"], self.subject("ADMIN_FORCED_TEST"))
        self.assertEqual(provenance["composition_trust"], UNROOTED)
        self.assertNotIn("claim", result.event_receipt)
        self.assertEqual(result.planning_context, planning)
        signal = result.affective_modulation_signal
        self.assertEqual(signal["authority_context_trust"], UNROOTED)
        self.assertEqual(signal["event_lineage"]["event_digest"], result.event_receipt["event_digest"])
        self.assertGreater(signal["target_modulation_strength"]["valuation"], 0.0)
        self.assertEqual(signal["evidence_effect"], "NONE")
        self.assertEqual(len(state_rows), 1)
        self.assertEqual(state_rows[0]["lifecycle_status"], "HISTORICAL")
    ...
    def test_cycle_rejects_legacy_authorized_boolean_surface(self):
        ...
        with self.assertRaises(TypeError):
            cycle.force_admin_test(authorized=True, planning_state={"truth": 0.9})
