...
class VeraAffectiveResumeTokenRestoreTests(unittest.TestCase):
    ...
    def test_restore_api_requires_external_resume_token(self):
        ...
        self.assertIn("expected_resume_token", inspect.signature(restore_host_from_state_row).parameters)
    ...
    def test_lower_restore_accepts_exact_externally_retained_frontier(self):
        checkpoint, row, token = self.make_row(state_version=7)
        contract_text, binding = self.contract_text_and_binding()
    ...
    def test_lower_restore_rejects_same_checkpoint_with_changed_provider_version(self):
        checkpoint, row, token = self.make_row(state_version=7)
        replayed = dict(row)
        replayed["state_version"] = 8
        self.assertEqual(replayed["checkpoint_sha256"], token["checkpoint_sha256"])
