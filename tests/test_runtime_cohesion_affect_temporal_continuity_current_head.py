...
class TemporalContinuityCurrentHeadTests(unittest.TestCase):
    ...
    def test_claimed_duration_cannot_outrun_trusted_elapsed_interval(self):
        runtime = self.make_runtime()
        # Establish a prior qualifying observation without crediting any time.
    ...
    def test_one_strong_observation_cannot_back_credit_a_long_unobserved_gap(self):
        runtime = self.make_runtime()
        # Zero-time observations may raise activation/coherence, but establish no
        # observed temporal continuity.
        for _ in range(6):
            runtime.apply_stimulus(
                self.strong_appraisal(duration_ms=0),
                elapsed_seconds=0.0,
        ...
        self.assertLess(
            ...
            minimum_window,
            "a single current qualifying sample may not retroactively classify an unobserved gap as sustained coherence",
        )
        self.assertIsNone(runtime.last_event_receipt)
