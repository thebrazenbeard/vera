...
class SpecialistTopologyV1Tests(unittest.TestCase):
    ...
    def test_currentness_refresh_replaces_closed_pr120_with_current_main_sd1(self):
        data = self.load_topology()
        refresh = data["currentness_refresh"]
        expected_main = "8ba7d1e23da3da4ca3a7dcd4ed94da237a656e37"
        expected_subject = (
            f"main@{expected_main}:architecture/cohesion/"
            "VERA_SEXUAL_DRIVE_COMPONENT_V1.json"
        )
        self.assertEqual(expected_main, refresh["vera_main"])
        self.assertEqual(expected_subject, refresh["current_sd1_subject"])
        self.assertEqual(
            "CLOSED_UNMERGED_SUPERSEDED_BY_CURRENT_MAIN_SD1_BINDING",
            refresh["predecessor_sd1_subject"]["disposition"],
        )

        edges = {item["edge_id"]: item for item in data["dependency_edges"]}
        self.assertEqual(
            expected_subject,
            edges["SEXUALITY_SD1_TO_VERA_COHESION"]["consumer"]["subject"],
        )
        self.assertEqual(
            expected_subject,
