...
def assertion(document, route):
    return next(
        item for item in document["ownership_assertions"]
        if item["route"] == route
    )


def materialize_declared_artifacts(root: Path, document: dict) -> None:
    for item in document["owners"]:
...
class WorkstreamCompatibilityTests(unittest.TestCase):
    def test_valid_matrix_passes(self):
        ...
        validate_semantics(document, registry(), ROOT)
    ...
    def test_same_source_and_target_rejected(self):
        document = matrix()
        document["interfaces"][0]["target_route"] = document["interfaces"][0][
            "source_route"
        ]
    ...
    def test_duplicate_directional_interface_pair_rejected(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "duplicates directional interface pair"
        ):
            validate_semantics(document, registry())

    def test_duplicate_interface_id_rejected(self):
        document = matrix()
        document["interfaces"][1]["interface_id"] = document["interfaces"][0][
            "interface_id"
        ]
    ...
    def test_duplicate_adapter_rejected(self):
        document = matrix()
        document["interfaces"][1]["required_adapter"] = document["interfaces"][0][
            "required_adapter"
        ]
    ...
    def test_duplicate_finding_id_rejected(self):
        document = matrix()
        document["interfaces"][1]["findings"][0]["finding_id"] = document[
            "interfaces"
        ][0]["findings"][0]["finding_id"]
        with self.assertRaisesRegex(ValueError, "duplicate finding_id"):
            validate_semantics(document, registry())

    def test_source_contract_must_belong_to_source(self):
        document = matrix()
        document["interfaces"][0]["source_contract_ids"] = [
            "VERA_COORDINATION_BUS_V1"
        ]
    ...
    def test_target_contract_must_belong_to_target(self):
        document = matrix()
        document["interfaces"][0]["target_contract_ids"] = [
            "VERA_MEMORY_CROSS_CHAT_CONTRACT_V1"
        ]
    ...
    def test_identity_time_anchor_artifact_omission_rejected(self):
        document = matrix()
        interface(document, "VERA-IFACE-001")["acceptance_evidence"][
            "source_artifacts"
        ].remove("architecture/identity/VERA_IDENTITY_TEMPORAL_ANCHOR_V1.json")
        with self.assertRaisesRegex(ValueError, "temporal-anchor binding"):
            validate_semantics(document, registry())
    ...
    def test_identity_time_validator_omission_rejected(self):
        ...
        interface(document, "VERA-IFACE-001")["acceptance_evidence"][
            "target_artifacts"
        ].remove("scripts/validate_identity_temporal_anchor.py")
        with self.assertRaisesRegex(ValueError, "temporal-anchor binding"):
            validate_semantics(document, registry())

    def test_source_artifact_must_belong_to_source(self):
        document = matrix()
        interface(document, "VERA-IFACE-004")["acceptance_evidence"][
            "source_artifacts"
    ...
    def test_unregistered_integration_artifact_cannot_self_certify(self):
        document = matrix()
        interface(document, "VERA-IFACE-010")["acceptance_evidence"][
            "source_artifacts"
    ...
    def test_authority_owner_substitution_rejected(self):
        document = matrix()
        interface(document, "VERA-IFACE-006")[
            "authority_owner"
        ] = "workstream/initiatives"
        with self.assertRaisesRegex(
            ValueError, "authority owner must be workstream/identity"
        ):
            validate_semantics(document, registry())

    def test_permission_owner_substitution_rejected(self):
        document = matrix()
        interface(document, "VERA-IFACE-004")[
            "permission_owner"
        ] = "workstream/time"
        with self.assertRaisesRegex(
            ValueError, "permission owner must be workstream/memory"
        ):
            validate_semantics(document, registry())
    ...
    def test_initiatives_interface_requires_abstention_and_no_execution(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "selection, abstention, and no-execution"
        ):
            validate_semantics(document, registry())
    ...
    def test_coordination_to_memory_requires_operational_boundary(self):
        document = matrix()
        interface(document, "VERA-IFACE-007")["capabilities"] = [
            "OPERATIONAL_EVENT_REFERENCE"
        ]
        with self.assertRaisesRegex(
            ValueError, "operational and non-promoting"
        ):
            validate_semantics(document, registry())

    def test_integration_cannot_claim_component_semantics(self):
        document = matrix()
        interface(document, "VERA-IFACE-010")["capabilities"] = [
            "COMPATIBILITY_FINDING"
        ]
        with self.assertRaisesRegex(
            ValueError, "disclaim component-semantic ownership"
        ):
            validate_semantics(document, registry())
    ...
    def test_registry_predecessor_lineage_is_immutable(self):
        document = matrix()
        document["registry_predecessor_head"] = "0" * 40
        with self.assertRaisesRegex(ValueError, "predecessor registry lineage"):
            validate_semantics(document, registry())

    def test_active_registry_digest_detects_mutation(self):
        document = matrix()
        changed_registry = registry()
        changed_registry["lifecycle_status"] = "MUTATED_BUT_SEMANTICALLY_UNREAD"
        with self.assertRaisesRegex(ValueError, "active registry digest"):
            validate_semantics(document, changed_registry)

    def test_stale_digest_reuse_rejected(self):
        document = matrix()
        changed_registry = registry()
        changed_registry["lifecycle_status"] = "OTHER_DRAFT"
        self.assertNotEqual(
            document["active_registry_sha256"],
            canonical_json_sha256(changed_registry),
        )
        with self.assertRaisesRegex(ValueError, "active registry digest"):
            validate_semantics(document, changed_registry)

    def test_memory_request_idempotency_evidence_omission_rejected(self):
        document = matrix()
        interface(document, "VERA-IFACE-008")["acceptance_evidence"][
            "source_artifacts"
        ].remove(
            "supabase/migrations/20260731003000_add_memory_request_idempotency.sql"
        )
        with self.assertRaisesRegex(ValueError, "Memory durability artifacts"):
            validate_semantics(document, registry())
    ...
    def test_memory_receipt_recovery_evidence_omission_rejected(self):
        ...
        interface(document, "VERA-IFACE-008")["acceptance_evidence"][
            "source_artifacts"
        ].remove("supabase/tests/verify_neutral_v3_memory_receipt_recovery.sql")
        with self.assertRaisesRegex(ValueError, "Memory durability artifacts"):
            validate_semantics(document, registry())

    def test_memory_durability_check_omission_rejected(self):
        document = matrix()
        interface(document, "VERA-IFACE-002")["acceptance_evidence"][
            "required_checks"
        ].remove("Memory durability review corrections")
        with self.assertRaisesRegex(ValueError, "Memory durability checks"):
            validate_semantics(document, registry())

    def test_closed_memory_component_gate_finding_rejected(self):
        document = matrix()
        interface(document, "VERA-IFACE-008")["findings"][0][
            "code"
        ] = "DURABLE_REQUEST_EVIDENCE_PENDING_COMPONENT_GATE"
        with self.assertRaisesRegex(ValueError, "unresolved assembly adapter"):
            validate_semantics(document, registry())

    def test_interface_evidence_must_exist_on_exact_tree(self):
        document = matrix()
        source_registry = registry()
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            materialize_declared_artifacts(root, source_registry)
            (root / "protocol/initiative_kernel.py").unlink()
            with self.assertRaisesRegex(ValueError, "missing repository artifact"):
                validate_semantics(document, source_registry, root)
    ...
    def test_required_negative_boundary_cannot_be_removed(self):
        document = matrix()
        assertion(document, "workstream/coordination")["does_not_own"] = [
            "MERGE_AUTHORITY"
        ]
    ...
    def test_registry_classification_mismatch_rejected(self):
        document = matrix()
        assertion(document, "workstream/memory")[
